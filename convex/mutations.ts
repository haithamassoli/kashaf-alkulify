...
export const upsertLessonByKey = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing === null) {
      ...
      return { created: true, id, skipped: false };
    }
...
export const acquirePipelineStage = mutation({
  args: { owner: v.string(), runId: v.string(), stage: v.string() },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("pipelineLocks")
...
export const upsertChannel = mutation({
  args: { title: v.string(), username: v.string() },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("channels")
    ...
    return { created: true, id, lastMessageId: 0 };
  },
});
...
export const finishPipelineRun = mutation({
  ...
  handler: async (ctx, { runId, ...counts }) => {
    ...
    if (existing === null) {
      throw new Error(`no pipelineRuns row for ${runId}`);
    }
...
export const recordFailure = mutation({
  args: { error: v.string(), refKey: v.string(), stage: v.string() },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("failures")
    ...
    return { attempts, id: existing._id };
  },
});

export const resolveFailure = mutation({
  args: { refKey: v.string(), stage: v.string() },
  handler: async (ctx, args) => {
    const rows = await ctx.db
      .query("failures")
      .withIndex("by_stage_ref", (q) =>
